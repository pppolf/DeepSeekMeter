...
11. **移动端同样零第三方依赖**：iOS/Android 业务逻辑零第三方依赖；系统框架（URLSession / SwiftUI / WebKit / Security / WidgetKit，以及 Android 的 Compose / HttpURLConnection / WorkManager 等）与平台官方工具不视为第三方（与 Windows 版 WebView2 例外同理）。其中 WorkManager 为 androidx 官方后台调度库，准入已经 Issue 讨论（红线 1，见 Issue #11），仅用于 `:app` 层后台刷新，`:core` 保持零 AndroidX 依赖

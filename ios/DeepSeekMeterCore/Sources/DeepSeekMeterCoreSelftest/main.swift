...
// 轻量自测（不依赖 XCTest，CLT 环境可直接运行；对齐 Scripts/selftest/main.swift）
// 运行：swift run --package-path ios/DeepSeekMeterCore DeepSeekMeterCoreSelftest
var failures = 0
...
/// 镜像 macOS PopoverView 的千分位格式化辅助（UI 层 helper，这里保留断言以锁住回归）
...
check(CountString(1130) == "1,130", "千分位")
...
// 11. PlatformService（可注入 URLSession）：请求头与错误归一化
...
StubURLProtocol.handler = { request in
    check(request.url?.host == "platform.deepseek.com", "请求域名正确")
    check(request.value(forHTTPHeaderField: "Authorization") == "Bearer test-token-abc", "Authorization 头")
    check(request.value(forHTTPHeaderField: "Referer") == "https://platform.deepseek.com/usage", "Referer 头")
    check(request.value(forHTTPHeaderField: "Origin") == "https://platform.deepseek.com", "Origin 头")
    check(request.value(forHTTPHeaderField: "User-Agent")?.isEmpty == false, "User-Agent 头")
...
Task {
    let service = PlatformService(session: stubSession())
    let user = try await service.fetchCurrentUser(token: "test-token-abc")
    check(user.email == "dev@example.com" && user.currency == "USD", "fetchCurrentUser 返回 email/currency")
    check(PlatformError.emptyToken.message == "请先在设置中填写平台 Token", "空 token 中文文案")

    // 业务错误 biz_code != 0 -> PlatformError.api
    let bizErr = """
{"code":0,"msg":"","data":{"biz_code":40002,"biz_msg":"token 失效","biz_data":null}}
"""
    StubURLProtocol.handler = { request in
        let resp = HTTPURLResponse(url: request.url!, statusCode: 200, httpVersion: nil, headerFields: nil)!
        return (resp, Data(bizErr.utf8))
    }
    do {
        _ = try await service.fetchCurrentUser(token: "bad")
        check(false, "biz_code!=0 应抛错")
    } catch let error as PlatformError {
        check(error.message == "平台 Token 无效或已过期，请重新获取", "40002 过期文案")
        check(error.errorDescription == error.message, "LocalizedError 对齐")
    } catch {
        check(false, "异常类型不符：\(error)")
    }

    // HTTP 非 200 -> PlatformError.http
    StubURLProtocol.handler = { request in
    ...
    do {
        _ = try await service.fetchCurrentUser(token: "bad")
        check(false, "HTTP 500 应抛错")
    } catch let error as PlatformError {
        check(error.message == "用量获取失败（HTTP 500）", "HTTP 500 文案")
    } catch {
        check(false, "异常类型不符：\(error)")
    }
    ...
    do {
        ...
        check(false, "空 token 应抛错")
    } catch let error as PlatformError {
        check(error.message == "请先在设置中填写平台 Token", "空 token 文案")
    } catch {
        check(false, "异常类型不符：\(error)")
    }

    // 解码失败 -> PlatformError.decoding
    StubURLProtocol.handler = { request in
        let resp = HTTPURLResponse(url: request.url!, statusCode: 200, httpVersion: nil, headerFields: nil)!
    ...
    do {
        ...
        check(error.message.hasPrefix("用量解析失败："), "解码失败文案前缀")
    } catch {
        check(false, "异常类型不符：\(error)")
    }
...
// 保持进程存活直到 Task 完成（顶层 async 环境下 Task 会正常等待）
RunLoop.main.run()
